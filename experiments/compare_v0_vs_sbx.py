...
def run_mode(mode: str, args, setup, Zref):
    cfg = MOEADConfig(
        n_obj = 2,
        n_var = args.n_var,
        pop_size = args.N,
    ...
    out = moead_run(cfg, evaluate_fn = zdt1, W = setup.W, B = setup.B, xl = xl, xu = xu, reference_Z = Zref)
    return out
...
def main():
    ...
    out_v0 = run_mode("placeholder", args, setup, Zref)
    out_sbx = run_mode("sbx", args, setup, Zref)

    print(f"Final IGD (v0 / placeholder): {out_v0['history_igd'][-1]:.6f}")
    print(f"Final IGD (SBX + poly):       {out_sbx['history_igd'][-1]:.6f}")
    ...
    plt.title("ZDT1 – v0 vs SBX (final Pareto fronts)")
    plt.show()
    ...
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.title("ZDT1 – IGD over generations")
    plt.show()
