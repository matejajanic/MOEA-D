...
def run_once(mode: str, seed: int, args, setup, Zref) -> float:
    cfg = MOEADConfig(
        n_obj=2,
        n_var=args.n_var,
        pop_size=args.N,
...
def main():
    ...
    p.add_argument("--csv", type=str, default="report/igd_zdt1.csv")
    args = p.parse_args()
    ...
    print(f"Running ZDT1 IGD table: N = {args.N}, T = {args.T}, n_gen = {args.n_gen}, runs = {len(seed_list)}")

    for s in seed_list:
        v0 = run_once("placeholder", s, args, setup, Zref)
        sbx = run_once("sbx", s, args, setup, Zref)
        igd_v0.append(v0)
        igd_sbx.append(sbx)
